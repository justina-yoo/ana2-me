...
export default function SugarGlycationBody({ lang }) {
  const isKo = lang === 'ko';
  return (
      ...
      <ArtSection>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <ArtCallout icon="✨" title={isKo ? '나이아신아마이드 (Niacinamide, 비타민 B3)' : 'Niacinamide (Vitamin B3)'} borderColor="rgba(45,90,61,0.2)" bgColor="rgba(45,90,61,0.04)">
            {isKo ? '이미 널리 사용되는 성분이지만, 당화 방지 효과는 덜 알려져 있어요. 나이아신아마이드는 NADPH 생성을 촉진해서 산화 스트레스를 줄이고, AGE-RAGE 경로에 의한 염증 반응을 완화해요. 동시에 콜라겐 합성도 촉진하니까 당화로 손실된 콜라겐을 보충하는 역할까지 해요.' : 'Already widely used, but its anti-glycation properties are less known. Niacinamide boosts NADPH production to reduce oxidative stress and dampens the inflammatory response triggered by the AGE-RAGE pathway. It also associated with firmer-looking skin — helping replenish what glycation destroys.'}
          </ArtCallout>
