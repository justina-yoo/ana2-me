...
export function fetchFeaturedArticles() {
...
export function fetchLatestArticles(limit, offset) {
...
export function toggleFeatured(id, value) {
...
export function fetchAnalyzeData(productIds) {
  var cacheKey = productIds.slice().sort().join(',');
  if (_analyzeCache[cacheKey]) {
    return Promise.resolve(_analyzeCache[cacheKey]);
  }
  ...
  var allRowsPromise = _allRowsCache
    ? Promise.resolve(_allRowsCache)
